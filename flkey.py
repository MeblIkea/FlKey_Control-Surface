...
class FlKey(InstrumentControlMixin, NovationBase):
    ...
    def on_identified(self, midi_bytes):
        ...
        super(FlKey, self).on_identified(midi_bytes)

    def port_settings_changed(self):
        self._auto_arm.set_enabled(False)
        super(FlKey, self).port_settings_changed()

    def _create_components(self):
        super(FlKey, self)._create_components()
    ...
    def _create_session_layer(self):
        return super(FlKey, self)._create_session_layer() + Layer(scene_launch_buttons="scene_launch_buttons")
    ...
    def _create_background(self):
        self._background = BackgroundComponent(name="Background",
                                               ...
                                               layer=Layer(secondary_up_button="secondary_up_button",
                                                           ...
                                                           unused_matrix=(
                                                           self._elements.device_select_matrix.submatrix[:, 1:]),
    ...
    def _create_view_control(self):
        ...
        self._session_ring_selection_linking = self.register_disconnectable(
            SessionRingSelectionLinking(session_ring=(self._session_ring),
    ...
    def _create_recording_modes(self):
        super(FlKey, self)._create_recording_modes()
        self._recording_modes.add_mode("arrange", AddLayerMode(self._transport, Layer(record_button="record_button")))
        self._FlKey__on_main_view_changed.subject = self.application.view
    ...
    def _create_pot_or_fader_modes(self, modes_type_name):
        ...
        def add_pot_or_fader_mixer_mode(parameter_name):
            modes.add_mode(parameter_name, (
                partial(getattr(self._mixer, "set_{}_parameter_name".format(modes_type_name)),
                        parameter_name.replace("_", " ").title()),
                AddLayerMode(self._mixer, Layer(**{("{}_controls".format(parameter_name)): elements_name,
                                                   ("{}_parameter_name_displays".format(
                                                       modes_type_name)): name_displays_element_name,

                                                   ("{}_displays".format(
                                                       parameter_name)): value_displays_element_name}))))
    ...
    def _create_stop_solo_mute_modes(self):
        ...
        self._stop_solo_mute_modes.add_mode("stop",
                                            (AddLayerMode(self._session,
                                                          Layer(stop_track_clip_buttons=lower_matrix_row))),
        ...
        self._FlKey__on_stop_solo_mute_mode_changed.subject = self._stop_solo_mute_modes
    ...
    def _create_pad_modes(self):
        ...
        self._FlKey__on_pad_mode_changed.subject = self._pad_modes
        self._FlKey__on_pad_mode_byte_changed.subject = self._pad_modes
    ...
    def _target_track_changed(self):
        super(FlKey, self)._target_track_changed()
    ...
    def _extract_product_id_bytes(self, midi_bytes):
        id_bytes = super(FlKey, self)._extract_product_id_bytes(midi_bytes)
